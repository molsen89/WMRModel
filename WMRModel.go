...
func main() {
	...
	// User inputs for Well-Mixed Room Model (WMR), room dimensions, and shift parameters
	// All units are in feet, minutes, and mg/m3
	// Ventilation rate is in ft3/min
	...
	// Calculations for room dimensions, steady-state concentration, and time-weighted average concentration
	...
	// Output results and check against occupational exposure limit
	fmt.Printf("The steady-state concentration at the end of the process is: %.2f mg/m3\n", WMR_SteadyState_ProcT)

	// Check if steady-state concentration exceeds OEL
